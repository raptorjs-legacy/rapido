function requireTimed(module) {
    ...
    try
    {
        ...
        // console.log(module + ' loaded at ' + (Date.now() - start) + 'ms')    
    }
}
...
var raptor = requireTimed('raptor');

requireTimed('raptor/logging').configure({
    loggers: {
        ROOT: {level: 'WARN'}
    }
});

var logger = requireTimed('raptor/logging').logger('rapido');
...
exports.create = function() {
    ...
    var rapido = {
        ...
        scaffold: function(config) {
            requireTimed('./scaffolding').generate(config, rapido);
        },
        $0: $0,
        ...
        init: function() {
            ...
            if (!config) {
                ...
                raptor.forEachEntry(_hiddenCommands, function(name, value) {
            ...
            function invokeCommand(command) {
                ...
                try
                {
                    ...
                    if (options) {
                        ...
                        if (commandArgs.help) {
                            command.showHelp(optimistCommand);
                            return;
                        }
