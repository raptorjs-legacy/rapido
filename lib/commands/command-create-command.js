...
module.exports = {
    ...
    run: function(args, config, rapido) {
        ...
        var scaffoldDir = config["scaffold.command.dir"];
        if (!scaffoldDir) {
            console.error('"scaffold.command.dir" not defined in rapido.json config file');
            return;
        }
        ...
        raptor.forEachEntry(projectConfigInfo.config, function(key, value) {
            if (key.startsWith('stack.')) {
                defaultStack = key.substring('stack.'.length);
            }
        }, this);
        ...
        prompt.get(
            ...
            function (err, result) {
                ...
                var stackKey = 'stack.' + stackName; 
                var stackConfig = projectConfig[stackKey] || (projectConfig[stackKey] = {
                    description: stackName
                });

                var relativePath = path.relative(projectConfigFile.getParent(), outputFile.getAbsolutePath());

                stackConfig['command.' + commandName] = {
                    description: description,
                    file: relativePath
                }
